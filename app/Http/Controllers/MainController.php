...
use Illuminate\Support\Facades\Redirect; // Добавили для фикса ошибки
...
class MainController extends Controller {
    ...
    public function cabinet() {
        /** @var User $user */
        $user = Auth::user();
        if (!$user || $user->role !== 'lead') abort(403);
    ...
    public function createMC() {
        ...
        $busySlots = \App\Models\MasterClass::where('user_id', $user->id)
            ->get(['date', 'time_slot'])
    ...
    // Сохранение в БД
    public function storeMC(\Illuminate\Http\Request $request) {
        ...
        \App\Models\MasterClass::create($data);
    ...
    $mc = \App\Models\MasterClass::with(['category', 'instructor'])->findOrFail($id);
    ...
    public function editMC($id) {
        ...
        if ($mc->user_id !== Auth::id()) abort(403);
